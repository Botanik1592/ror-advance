...
feature 'Create comment', %q{
  ...
  scenario 'Authenticated user leave comment for a question', js: true do
    sign_in(user)
    visit question_path(question)

    within "#question-#{question.id}-comments" do
      fill_in "new-comment-body-#{question.id}", with: 'This is test comment'
      click_on 'Save'
    end
  ...
  scenario 'Authenticated user leave invalid comment for a question', js: true do
    sign_in(user)
    visit question_path(question)

    within "#question-#{question.id}-comments" do
      fill_in "new-comment-body-#{question.id}", with: 'T1'
      click_on 'Save'
    end
